document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeApp = async (userData) => {
        ...
        attachAllEventListeners(); // Listenery przypisane po załadowaniu danych
    ...
    const checkLoginStatus = async () => {
        ...
        if (!token) {
            console.log('No token found in localStorage. Showing login form.');
            // Upewnijmy się, że listener logowania jest aktywny nawet bez tokena
        ...
        try {
            ...
            if (response.ok) {
                ...
                attachLoginListeners(); // Token nieważny, podepnij listenery logowania
            }
        } catch (error) { 
            console.error('Błąd weryfikacji tokenu:', error); 
            attachLoginListeners(); // Błąd sieci, podepnij listenery logowania
        }
    };
    ...
    // --- NOWA FUNKCJA DO PODPINANIA TYLKO LISTENERÓW LOGOWANIA ---
    function attachLoginListeners() {
        console.log('Attaching login listeners...');
        if (elements.loginBtn) {
            elements.loginBtn.removeEventListener('click', attemptLogin); // Usuń stary na wszelki wypadek
    ...
    function attachAllEventListeners() {
        console.log('Attaching ALL application listeners...');
        // Tutaj podpinamy wszystkie listenery OPRÓCZ logowania, które ma swoją funkcję
    ...
    // ... (reszta funkcji: loadDataFromServer, switchTab, etc. bez zmian) ...
    
    async function loadDataFromServer() { console.log('Ładowanie bazy produktów...'); function fetchAndParseCsv(filename) { return fetch(filename).then(r => r.ok ? r.arrayBuffer() : Promise.reject(new Error(`Błąd sieci: ${r.statusText}`))).then(b => new TextDecoder("Windows-1250").decode(b)).then(t => new Promise((res, rej) => Papa.parse(t, { header: true, skipEmptyLines: true, complete: rts => res(rts.data), error: e => rej(e) }))); } await Promise.all([fetchAndParseCsv('produkty.csv'), fetchAndParseCsv('produkty2.csv')]) .then(([data1, data2]) => { const mapData = p => ({ kod_kreskowy: String(p.kod_kreskowy || "").trim(), nazwa_produktu: String(p.nazwa_produktu || "").trim(), cena: String(p.opis || "0").replace(',', '.').trim() || "0", opis: String(p.cena || "").trim() }); productDatabase = [...data1.map(mapData), ...data2.map(mapData)]; console.log(`Baza danych załadowana (${productDatabase.length} pozycji).`); }).catch(error => { console.error('Krytyczny błąd ładowania danych:', error); alert('BŁĄD: Nie udało się załadować bazy produktów.'); }); }
    function switchTab(newTab) { activeTab = newTab; [elements.lookupMode, elements.listBuilderMode, elements.adminPanel].forEach(el => { if(el) el.style.display = 'none'; }); [elements.tabLookupBtn, elements.tabListBuilderBtn].forEach(el => { if(el) el.classList.remove('active');}); if (newTab === 'lookup') { if(elements.lookupMode) elements.lookupMode.style.display = 'block'; if(elements.tabLookupBtn) elements.tabLookupBtn.classList.add('active'); } else if (newTab === 'listBuilder') { if(elements.listBuilderMode) elements.listBuilderMode.style.display = 'block'; if(elements.tabListBuilderBtn) elements.tabListBuilderBtn.classList.add('active'); } else if (newTab === 'admin') { if(elements.adminPanel) elements.adminPanel.style.display = 'block'; } }
    ...
    function clearCurrentList(askConfirm = true) { if (askConfirm && scannedItems.length > 0 && !confirm('Czy na pewno chcesz wyczyścić bieżące zamówienie? Ta operacja usunie również aktywną, zapamiętaną listę.')) { return; } scannedItems = []; elements.clientNameInput.value = ''; elements.additionalInfoInput.value = ''; localStorage.removeItem('activeListId'); renderScannedList(); showToast("Utworzono nową, czystą listę."); }
    async function saveCurrentList() { const listName = prompt("Podaj nazwę dla zapisywanego zamówienia:", elements.clientNameInput.value || `Zamówienie ${getSafeFilename()}`); if (!listName) return null; try { const response = await fetch('/api/data/savelist', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-auth-token': localStorage.getItem('token') }, body: JSON.stringify({ listName, items: scannedItems, clientName: elements.clientNameInput.value }) }); if (!response.ok) { const errData = await response.json(); throw new Error(errData.msg || "Błąd zapisu"); } const savedList = await response.json(); showToast(`Zamówienie "${listName}" zostało zapisane.`); localStorage.setItem('activeListId', savedList._id); return savedList; } catch (error) { alert(`Błąd: ${error.message}`); return null; } }
    async function loadListById(listId) { try { const response = await fetch(`/api/data/list/${listId}`, { headers: { 'x-auth-token': localStorage.getItem('token') } }); if (!response.ok) throw new Error("Błąd wczytywania listy"); const data = await response.json(); scannedItems = data.items; elements.clientNameInput.value = data.clientName || data.listName; renderScannedList(); localStorage.setItem('activeListId', listId); return data; } catch (error) { alert(error.message); localStorage.removeItem('activeListId'); return null; } }
    ...
    async function showSavedLists() { elements.savedListsModal.style.display = 'flex'; const container = elements.savedListsContainer; container.innerHTML = '<p>Ładowanie...</p>'; try { const response = await fetch('/api/data/lists', { headers: { 'x-auth-token': localStorage.getItem('token') } }); if (!response.ok) throw new Error("Błąd wczytywania list"); const lists = await response.json(); container.innerHTML = `<div style="padding-bottom: 15px; margin-bottom: 15px; border-bottom: 1px solid var(--border-color);"> <button id="importCsvBtn" class="btn btn-primary" style="background-color: var(--info-color); width: 100%;"> <i class="fa-solid fa-file-import"></i> Importuj zamówienie z pliku CSV </button> <input type="file" id="importCsvInput" accept=".csv" style="display: none;"> </div><h3>Zapisane listy:</h3>`; container.querySelector('#importCsvBtn').addEventListener('click', () => container.querySelector('#importCsvInput').click()); container.querySelector('#importCsvInput').addEventListener('change', handleFileImport); if (lists.length === 0) { container.innerHTML += '<p>Brak zapisanych zamówień.</p>'; return; } const listContainer = document.createElement('ul'); listContainer.style.cssText = 'list-style: none; padding: 0;'; lists.forEach(list => { const li = document.createElement('li'); li.style.cssText = 'display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid var(--border-color); flex-wrap: wrap; gap: 10px;'; li.innerHTML = `<span>${list.listName} <small>(autor: ${list.user?.username || 'usunięty'}, ost. zapis: ${new Date(list.updatedAt).toLocaleDateString()})</small></span> <div style="display: flex; gap: 5px; flex-wrap: wrap;"> <button class="btn-primary load-list-btn" data-id="${list._id}">Wczytaj</button> <button class="pick-order-btn" data-id="${list._id}" data-name="${list.listName}" style="background-color: var(--warning-color);">Kompletuj</button> <button class="btn-danger delete-list-btn" data-id="${list._id}">Usuń</button> </div>`; listContainer.appendChild(li); }); container.appendChild(listContainer); } catch (error) { container.innerHTML = `<p style="color:var(--danger-color)">${error.message}</p>`; } }
    async function handleSavedListAction(e) { const target = e.target.closest('button'); if (!target) return; const listId = target.dataset.id; if (target.classList.contains('load-list-btn')) { if (!confirm("Czy na pewno wczytać listę? Obecne zamówienie zostanie nadpisane.")) return; const loadedList = await loadListById(listId); if (loadedList) { elements.savedListsModal.style.display = 'none'; showToast(`Wczytano listę: ${loadedList.listName}`); } } else if (target.classList.contains('delete-list-btn')) { if (!confirm("Czy na pewno usunąć tę listę?")) return; try { const response = await fetch(`/api/data/list/${listId}`, { method: 'DELETE', headers: { 'x-auth-token': localStorage.getItem('token') } }); const data = await response.json(); if (!response.ok) throw new Error(data.msg || "Błąd usuwania listy"); showToast("Lista usunięta."); await showSavedLists(); } catch (error) { alert(error.message); } } else if (target.classList.contains('pick-order-btn')) { elements.savedListsModal.style.display = 'none'; await startPicking(listId, target.dataset.name); } }
    async function handleFileImport(event) { const file = event.target.files[0]; if (!file) return; const listName = prompt("Podaj nazwę dla importowanego zamówienia:", file.name.replace(/\.csv$/i, '')); if (!listName) { event.target.value = ''; return; } Papa.parse(file, { delimiter: ";", skipEmptyLines: true, complete: async (results) => { const itemsMap = new Map(); results.data.forEach(row => { const ean = row[0]?.trim(); const quantity = parseInt(row[1]?.trim(), 10); if (ean && !isNaN(quantity) && quantity > 0) itemsMap.set(ean, (itemsMap.get(ean) || 0) + quantity); }); const importedItems = Array.from(itemsMap, ([ean, quantity]) => { let p = productDatabase.find(prod => prod.kod_kreskowy === ean || prod.nazwa_produktu === ean); if (!p) p = { kod_kreskowy: ean, nazwa_produktu: ean, opis: ean, cena: "0" }; return { ean: p.kod_kreskowy, name: p.nazwa_produktu, description: p.opis, quantity, price: p.cena }; }); try { const response = await fetch('/api/data/savelist', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-auth-token': localStorage.getItem('token') }, body: JSON.stringify({ listName, items: importedItems, clientName: listName }) }); if (!response.ok) { const errData = await response.json(); throw new Error(errData.msg || "Błąd zapisu"); } showToast(`Zamówienie "${listName}" zostało zaimportowane i zapisane.`); await showSavedLists(); } catch (error) { alert(`Błąd: ${error.message}`); } }, error: (err) => { alert(`Błąd parsowania pliku CSV: ${err.message}`); } }); event.target.value = ''; }
    async function loadAllUsers() { try { const response = await fetch('/api/admin/users', { headers: { 'x-auth-token': localStorage.getItem('token') } }); if (!response.ok) throw new Error('Nie można załadować użytkowników'); const users = await response.json(); elements.allUsersList.innerHTML = users.map(user => ` <div class="user-item"> <div class="user-info"> <strong>${user.username}</strong> <span class="status">Rola: ${user.role} | Status: ${user.isApproved ? 'Zatwierdzony' : 'Oczekujący'}</span> </div> <div class="user-actions"> ${!user.isApproved ? `<button class="approve-user-btn" data-userid="${user._id}" data-username="${user.username}">Zatwierdź</button>` : ''} <button class="edit-user-btn" data-userid="${user._id}" data-username="${user.username}">Zmień hasło</button> <button class="change-role-btn" data-userid="${user._id}" data-username="${user.username}" data-role="${user.role === 'admin' ? 'user' : 'admin'}">Zmień na ${user.role === 'admin' ? 'User' : 'Admin'}</button> <button class="delete-user-btn" data-userid="${user._id}" data-username="${user.username}">Usuń</button> </div> </div> `).join(''); } catch (error) { elements.allUsersList.innerHTML = `<p style="color:red;">${error.message}</p>`; } }
    ...
    async function handleChangePassword() { const newPassword = prompt("Wprowadź nowe hasło:"); if (!newPassword) return; try { const response = await fetch('/api/auth/change-password', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-auth-token': localStorage.getItem('token') }, body: JSON.stringify({ newPassword }) }); const data = await response.json(); if (!response.ok) throw new Error(data.msg || "Błąd zmiany hasła"); alert("Hasło zostało zmienione."); } catch (error) { alert(`Błąd: ${error.message}`); } }
    ...
    function handleDeleteInventoryItem(e) { const btn = e.target.closest('.delete-inv-item-btn'); if (btn) { inventoryItems.splice(btn.dataset.index, 1); renderInventoryList(); } if(e.target.classList.contains('quantity-in-table')) { e.preventDefault(); openNumpad(e.target, (newValue) => { inventoryItems[e.target.dataset.index].quantity = newValue; renderInventoryList(); }); } }
    function handleInventoryAdd() { const ean = elements.inventoryEanInput.value.trim(); const quantity = parseInt(elements.inventoryQuantityInput.value, 10); if (!ean || !quantity) return; const existing = inventoryItems.find(i => i.ean === ean); if (existing) existing.quantity += quantity; else inventoryItems.push({ ean: ean, name: 'Inwentaryzacja', quantity: quantity }); renderInventoryList(); elements.inventoryEanInput.value = ''; elements.inventoryQuantityInput.value = '1'; }
    ...
    async function startPicking(listId, listName) { try { const response = await fetch(`/api/data/list/${listId}`, { headers: { 'x-auth-token': localStorage.getItem('token') } }); if (!response.ok) throw new Error("Błąd wczytywania zamówienia do kompletacji"); currentPickingOrder = await response.json(); pickedItems = []; elements.pickingOrderName.textContent = `Kompletacja: ${listName}`; renderPickingView(); elements.pickingModule.style.display = 'flex'; } catch (error) { alert(error.message); } }
    function renderPickingView() { if(!currentPickingOrder) return; const toPickItems = currentPickingOrder.items.filter(item => !pickedItems.some(p => p.ean === item.ean)); elements.pickingTargetList.innerHTML = toPickItems.map(item => `<div class="pick-item" data-ean="${item.ean}" style="padding: 8px; cursor: pointer; border-bottom: 1px solid var(--border-color);"><strong>${item.name}</strong><br><small>${item.description}</small> (ilość: ${item.quantity})</div>`).join('') || "<p>Wszystko zebrane!</p>"; elements.pickingScannedList.innerHTML = pickedItems.map((item, index) => `<div class="picked-item" style="display: flex; justify-content: space-between; align-items: center; padding: 5px;"><span>${item.name} | ${item.description}</span><div><input type="number" value="${item.quantity}" class="picked-quantity-input" data-index="${index}" style="width: 60px; text-align: center;" readonly><button class="unpick-item-btn btn-icon" data-index="${index}" style="background: none; color: var(--danger-color);"><i class="fa-solid fa-arrow-left"></i></button></div></div>`).join(''); }
    function pickItemFromList(ean) { const item = currentPickingOrder.items.find(i => i.ean === ean); if (item) { openNumpad({ value: item.quantity }, (quantity) => { if (quantity > 0) moveItemToPicked(item, quantity); }); } }
    function moveItemToPicked(item, quantity) { const itemToMove = { ...item, quantity }; pickedItems.push(itemToMove); renderPickingView(); }
    function handlePickedItemClick(e) { const target = e.target.closest('button.unpick-item-btn, input.picked-quantity-input'); if (!target) return; const index = target.dataset.index; if (target.classList.contains('unpick-item-btn')) { pickedItems.splice(index, 1); renderPickingView(); } else if (target.classList.contains('picked-quantity-input')) { e.preventDefault(); openNumpad(target, (newQuantity) => { if(newQuantity >= 0) { if (newQuantity === 0) pickedItems.splice(index, 1); else pickedItems[index].quantity = newQuantity; renderPickingView(); } }); } }
    function handlePickingSearch(event) { const searchTerm = event.target.value.trim(); const resultsDiv = elements.pickingSearchResults; resultsDiv.style.display = 'none'; resultsDiv.innerHTML = ''; if(!searchTerm) return; const results = performSearch(searchTerm).filter(p => !pickedItems.some(pi => pi.ean === p.kod_kreskowy)); if(results.length > 0) { let listHtml = '<ul>'; results.forEach(p => { listHtml += `<li data-ean="${p.kod_kreskowy}">${p.opis} <small>(${p.nazwa_produktu})</small></li>`; }); listHtml += '</ul>'; resultsDiv.innerHTML = listHtml; resultsDiv.style.display = 'block'; } }
    ...
    function attachNumpadListeners() { /* Logika dla numpada */ }
    function openNumpad(targetInput, callback) { /* Logika dla numpada */ }
