document.addEventListener('DOMContentLoaded', () => {
    ...
    function attachAllEventListeners() {
        ...
        if (elements.menuAdminBtn) elements.menuAdminBtn.addEventListener('click', (e) => { e.preventDefault(); showAdminPanel(); });
    ...
    function exportToExcelDetailed() { if (scannedItems.length === 0) return; const headers = '"Kod produktu";"Nazwa";"EAN";"Ilość"'; const rows = scannedItems.map(item => `"${item.name || ''}";"${(item.description || '').replace(/"/g, '""')}";"${item.ean || ''}";"${item.quantity || 0}"`); const csvContent = `\uFEFF${headers}\n${rows.join('\n')}`; downloadFile(csvContent, 'text/csv;charset=utf-8;', `${getSafeFilename()}_szczegoly.csv`); }
    ...
    async function showSavedLists() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Błąd wczytywania list: ${errText}`);
            }
    ...
    function handleInventoryAdd() {
        ...
        else inventoryItems.push({ ean: ean, name: ean, quantity: quantity });
        renderInventoryList();
