document.addEventListener('DOMContentLoaded', () => {
    ...
    function addProductToList(code = null, quantity = null) {
        ...
        if (!productData) {
            productData = { kod_kreskowy: ean, nazwa_produktu: ean, opis: ean, cena: "0" };
        }
        const existingItem = scannedItems.find(item => item.ean === productData.kod_kreskowy);
        if (existingItem) {
            existingItem.quantity += qty;
        } else {
            scannedItems.push({ ean: productData.kod_kreskowy, name: productData.nazwa_produktu, description: productData.opis, quantity: qty, price: productData.cena });
        }
        renderScannedList();
    ...
    function handleListBuilderSearch(event) {
        ...
        if (results.length > 0) {
            let listHtml = '<ul>';
            results.forEach(p => {
                listHtml += `<li data-ean="${p.kod_kreskowy}">${p.opis} <small>(${p.nazwa_produktu})</small></li>`;
            });
    ...
    function displayProductListInLookup(products) {
        let listHtml = '<ul>';
        products.forEach(p => {
            listHtml += `<li data-product-json='${JSON.stringify(p)}'>${p.opis} <small>(${p.nazwa_produktu})</small></li>`;
        });
        listHtml += '</ul>';
    ...
    function renderScannedList() {
        ...
        [elements.exportCsvBtn, elements.exportExcelBtn, elements.printListBtn, elements.clearListBtn, elements.saveListBtn].forEach(btn => {
    ...
    function exportToExcelDetailed() {
        if (scannedItems.length === 0) return;
        const headers = '"Kod produktu";"Nazwa";"EAN";"Ilość";"Cena Jednostkowa"';
        const rows = scannedItems.map(item => {
            const priceFormatted = (parseFloat(item.price) || 0).toFixed(2).replace('.', ',');
            return `"${item.name || ''}";"${(item.description || '').replace(/"/g, '""')}";"${item.ean || ''}";"${item.quantity || 0}";"${priceFormatted}"`;
        });
    ...
    function clearCurrentList(askConfirm = true) {
        if (askConfirm && scannedItems.length > 0 && !confirm('Czy na pewno chcesz wyczyścić bieżące zamówienie? Ta operacja usunie również aktywną, zapamiętaną listę.')) {
            return;
        }
        scannedItems = [];
    ...
    async function saveCurrentList() {
        ...
        try {
            const response = await fetch('/api/data/savelist', {
                ...
                body: JSON.stringify({ listName, items: scannedItems, clientName: elements.clientNameInput.value })
            });
            if (!response.ok) {
    ...
    async function showSavedLists() {
        ...
        try {
            ...
            container.innerHTML = `<div style="padding-bottom: 15px; margin-bottom: 15px; border-bottom: 1px solid var(--border-color);">
                                     <button id="importCsvBtnInModal" class="btn btn-primary" style="background-color: var(--info-color); width: 100%;">
            ...
            container.querySelector('#importCsvBtnInModal').addEventListener('click', () => container.querySelector('#importCsvInputInModal').click());
            container.querySelector('#importCsvInputInModal').addEventListener('change', handleFileImport);

            if (lists.length === 0) {
                container.innerHTML += '<p>Brak zapisanych zamówień.</p>';
                return;
            }
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                const importedItems = Array.from(itemsMap, ([ean, quantity]) => {
                    ...
                    if (!p) {
                        p = { kod_kreskowy: ean, nazwa_produktu: ean, opis: ean, cena: "0" };
                    }
            ...
            error: (err) => {
                alert(`Błąd parsowania pliku CSV: ${err.message}`);
            }
        });
        event.target.value = '';
    }
    ...
    function verifyPicking() {
        if (!currentPickingOrder) return;
        const summary = { missing: [], extra: [], incorrectQuantity: [] };
